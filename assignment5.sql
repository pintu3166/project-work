...
update worker set email='ankitaha@gmailcom' where first_name='ankita';
set sql_safe_updates=0;

select count(*) from worker where first_name not in ('vipu','satish');
select * from worker where first_name not in ('vipu','satish');
select * from worker where first_name like '_____h';
select email, case when email like '%_@_%._%' then 'valid' else 'invalid' end as checking from worker;
